...
namespace cnpy {
    ...
    struct NpyArray {
        NpyArray(const std::vector<size_t>& _shape, size_t _word_size, bool _fortran_order, char _type_class) :
            shape(_shape), word_size(_word_size), fortran_order(_fortran_order), type_class(_type_class)
        {
            num_vals = 1;
    ...
    std::vector<char> create_npy_header(const std::vector<size_t>& shape, char type_class, size_t word_size);
    // void parse_npy_header(FILE* fp,size_t& word_size, std::vector<size_t>& shape, char& type_class, bool& fortran_order);
    // void parse_npy_header(unsigned char* buffer,size_t& word_size, std::vector<size_t>& shape, char& type_class, bool& fortran_order);
    // void parse_zip_footer(FILE* fp, uint16_t& nrecs, size_t& global_header_size, size_t& global_header_offset);
    npz_t npz_load(std::string fname);
    NpyArray npz_load(std::string fname, std::string varname);
    NpyArray npy_load(std::string fname);
    // refine
    void parse_npy_header(std::istream &is, 
                          size_t& word_size, std::vector<size_t>& shape, char &type_class, bool &fortran_order);
    void parse_zip_footer(std::istream &is,
    ...
    //         std::vector<char> npy_header = create_npy_header(shape, type_class, word_size);
    ...
    void npz_save(std::string zipname, std::string fname, const T* data, const std::vector<size_t>& shape, std::string mode = "w")
    {
        ...
        std::cout << std::hex << "local_header: nbytes:" << nbytes << " , fname size: " << fname.size() << " fname: "<< fname << std::endl;
        ...
        footer += (uint32_t) (global_header_offset + nbytes + local_header.size()); //offset of start of global headers, since global header now starts after newly written array
        footer += (uint16_t) 0; //zip file comment length
        std::cout << "footer: nrecs:" << nrecs + 1 << " global_header_size: " << global_header.size() << " all size:" << (global_header_offset + nbytes + local_header.size())
                  << std::dec << std::endl;
        //write everything
        // fwrite(&local_header[0],sizeof(char),local_header.size(),fp);
        // fwrite(&npy_header[0],sizeof(char),npy_header.size(),fp);
